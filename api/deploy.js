...
export default async function handler(req, res) {
  ...
  try {
    ...
    const id = `${safeName}-${shortId}`; // ex: testing-a1b2c3d4

    // FIX: upload as index.html so site.js finds it
    ...
    return res.status(200).json({
      success: true,
      url: `https://${id}.webidex.in`, // CLEAN - till webidex.in
      shortUrl: `https://webidex.in/s/${id}`,
      id
    });
  } catch (e) {
    console.error(e);
